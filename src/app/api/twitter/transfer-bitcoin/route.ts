...
export async function GET(request: Request) {
  try {
    ...
    const isReceiverIdNearAccount = searchParams.get('isReceiverIdNearAccount');
    ...
    if (!accountId || !amountInURL || !receiverId || !isReceiverIdNearAccount || !isAmountInBitcoin ) {
      console.log('some required parameters are missing');
      console.log({accountId, amountInURL, receiverId, isReceiverIdNearAccount, isAmountInBitcoin});
    ...
    const link = `https://wallet.bitte.ai/sign-transaction?transactions_data=${encodeURI(JSON.stringify(TransactionToSign))}&callbackUrl=${PLUGIN_URL}/api/twitter/relay-transaction?data=${encodeURIComponent(transactionData)}`;
    console.log({link});
