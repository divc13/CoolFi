...
export async function GET(request: Request) {
  try {
    ...
    const link = `https://app.mynearwallet.com/sign-message?message=${(JSON.stringify(messageString))}&nonce=${(transactionPayload.nonce)}&recipient=intents.near&callbackUrl=${PLUGIN_URL}/redirect?data=${encodeURIComponent(JSON.stringify(data))}`;
