...
export const fetchJsonZEC = async (url, params = {}, noWarnings = false) => {
    const apiKey = settings.crypto_api_key; // Replace this with your actual API key
  
    // Ensure headers exist and add x-api-key
    params.headers = {
      ...params.headers,
      "x-api-key": apiKey,
    };
...
export const convertZCash = (value, toUnit) => {
    if (toUnit === "zec") {
      // Convert satoshis to bitcoins and return as a string with fixed precision
      return (value / 100000000).toFixed(8).toString();
    } else if (toUnit === "zats") {
      ...
      throw new Error(
        'Invalid unit specified. Use "zec" for bitcoins or "zats" for zatoshis.'
      );
    }
  };
